...
export async function getParsedXML(url) {
    const xml = await fs.readFile(url);
    return getParsed(xml);
}
export async function orgCreate(testProjectName) {
...
export async function itDeploys(testProjectName) {
    await this.orgCreate(testProjectName);
    // push source
    const pushResult = await exec2JSON('sfdx force:source:push --json', {
        cwd: testProjectName
    });
